...
import { MoveHandlers } from '../data/moveHandlers.js';
...
export class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    //bulbasaur moves
    this.load.image('razorLeaf', 'assets/projectiles/razorLeaf.png');
    //maps
    ...
    //music
    this.load.audio('song1', 'assets/audio/planetWisp.mp3');
    this.load.audio('song2', 'assets/audio/pmd1.mp3');
  }
  ...
  showUpgradeMenu() {
    ...
    const moves = POKEMON_DATA[this.selectedPokemon?.toLowerCase()].moves; // ✅ Dynamic move list
  ...
  createOrbitals(config) {
    ...
    this.orbitalData = { ...config, angleOffset: 0 };

    for (let i = 0; i < config.count; i++) {
      const orb = this.physics.add.sprite(this.player.x, this.player.y, config.spriteKey);
      orb.setScale(config.size);
      orb.damage = config.damage;
      orb.setOrigin(0.5);
      orb.hitEnemies = new Set(); // ✅ Track what this orb already hit
      this.orbitals.add(orb);
    }
  }
  ...
  create() {
    ...
    function playNext(scene) {
      if (currentSong === 1) {
        music1 = scene.sound.add('song1', {volume: 0.25});
        music1.play();
        ...
        music2 = scene.sound.add('song2', {volume: 0.25});
        music2.play();
  ...
  update() {
    ...
    this.orbitals.children.each((orb) => {
      this.enemies.children.each((enemy) => {
      if (!orb.active || !enemy.active) return;

      if (!orb.hitEnemies.has(enemy) &&
        Phaser.Geom.Intersects.RectangleToRectangle(orb.getBounds(), enemy.getBounds())) {
          enemy.hp -= orb.damage;
  ...
  setupAutoAttack(pokemon) {
    const delay = 1000;
    const pokemonData = POKEMON_DATA[pokemon.toLowerCase()];
    const basicMove = pokemonData?.moves.find(m => m.isBasic);

    if (!basicMove) {
      console.warn(`No basic move found for ${pokemon}`);
      return;
    }
    ...
    this.time.addEvent({
      ...
      callback: () => {
        if (!this.player.active) return;
        // Avoid reapplying orbitals every second
if (basicMove.name === 'Razor Leaf') {
  if (!this.activeMoves.includes('Razor Leaf')) {
